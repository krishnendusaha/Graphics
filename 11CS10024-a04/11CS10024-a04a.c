...
//#####################CONFIGURE  THE CUBE #########################################################################
  ...
  //#####################CONFIGURE  THE CUBE ######################################################################### 
    ...
    for(k=0;k<3*STEPS;k++)
    {
        ...
        snprintf(fileNo,sizeof(fileNo),"%d",0);
        strcat(fileName,fileNo);
        strcat(fileName,".svg");
      
      float intensity[4],min_intensity=10000;  
      Vector N; 
      float x1_min=10000,x1_max=-10000;
      float y1_min=10000,y1_max=-10000;
      float p_x[4],p_y[4];
      ...
      for(i=0;i<3;i++)
       {
          if(face_to_project[i]>=0) 
           {
            ...
            for(j=0;j<4;j++){ x1_min=(array2[c[j]].x<x1_min)?array2[c[j]].x:x1_min; }
            for(j=0;j<4;j++){ y1_min=(array2[c[j]].y<y1_min)?array2[c[j]].y:y1_min; }
            for(j=0;j<4;j++){ x1_max=(array2[c[j]].x>x1_max)?array2[c[j]].x:x1_max; }
            for(j=0;j<4;j++){ y1_max=(array2[c[j]].y<y1_max)?array2[c[j]].y:y1_max; } 

            for(j=0;j<4;j++){ p_x[j]=(array2[c[j]].x-x1_min)/(x1_max-x1_min)*100;  p_y[j]=(array2[c[j]].y-y1_min)/(y1_max-y1_min)*100; }
          ...
          if(intensity[0]>intensity[2]){fprintf(graphic, "<linearGradient id=\"grad1\" x1=\"%f%c\" y1=\"%f%c\" x2=\"%f%c\" y2=\"%f%c\">\n",p_x[0],37,p_y[0],37,p_x[2],37,p_y[2],37); }
          ...
          if(intensity[1]>intensity[3]){fprintf(graphic, "<linearGradient id=\"grad2\" x1=\"%f%c\" y1=\"%f%c\" x2=\"%f%c\" y2=\"%f%c\">\n",p_x[1],37,p_y[1],37,p_x[3],37,p_y[3],37); }
